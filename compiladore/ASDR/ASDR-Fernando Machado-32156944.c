...
const char * MapaAtomos[40] = {"ERRO","INDENTIFICADOR","ABRE_PAR","ATRIBUICAO","FECHA_PAR","PONTO","PONTO_VIRGULA",
"VIRGULA","SUBTRACAO","ADICAO","DIVISAO","MULTIPLICACAO","OP_RELACIONAL","COMENTARIO","NUMERO_INTERIO","NUMERO_REAL",
"CARACTERE","AND","BEGIN","END","BOOLEAN","CHAR","DO","ELSE","FALSE","IF","INTEGER","MOD","NOT","OR","PROGRAM","READ",
"REAL","THEN","TRUE","WHILE","WRITE","EOS"};
...
int main( void )
{
	...
	printf("Analisando o seguinte:\n %s\n",buffer);
    char *iniBuffer = buffer;
    lookahead = obteratomo1();
    programa();
    if( lookahead.atomo == EOS )
        printf("palavras aceitas.\n");
    else
        printf("Erro sintatico 2.\n");

    printf("\nFim de programa.\n");
    // no final da analise libera a memoria alocada
    free(iniBuffer);
